...
class Adapter(nn.Module):
    def __init__(self, config):
        ...
        nx = config.n_embd
        self.adapter_design = config.adapter_design
...
class Block(nn.Module):
    def __init__(self, n_ctx, config, scale=False):
        ...
        if self.adapter_design == 2:
            self.adapter_block = Adapter(config)
            self.adapter_block2 = Adapter(config)
            self.forward = self.forward_twoAdapter
        elif self.adapter_design == 1:
            self.adapter_block = Adapter(config)
            self.forward = self.forward_oneAdapter




    def forward_twoAdapter(self,
        hidden_states,
        layer_past=None,
        attention_mask=None,
        head_mask=None,
        ...
        # residual connection
        attn_output = self.adapter_block(attn_output)
        ...
        feed_forward_hidden_states = self.adapter_block2(feed_forward_hidden_states)
    ...
    def forward_oneAdapter(
        ...
        if self.bottleneck > 0:
            hidden_states = self.adapter_block(hidden_states)
...
class GPT2LMHeadModelAdapter(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.task_mode = config._my_arg_task_mode
        if config._my_arg_tune_mode == 'finetune' or config._my_arg_tune_mode == 'adaptertune':
            self.finetune_mode = True
